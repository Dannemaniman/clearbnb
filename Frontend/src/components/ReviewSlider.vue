  ...
  <section class='section-slider' > 
    <div class='slider-container' > 
      <div class='arrow-left' @click='moveLeft' />
        <article class="review-card"  :class="`slide${String(Math.random()).replace(
        '.', '')}`" v-for="review of reviews"  :review="review" :key="review.id" @click="popReview(review.id)" :ref="review.id">
            ...
            <img class="conversation-icon" src="../../public/icons/conversation.png" @click="popReplySection(review)"/>
        </article>
...
export default {
  ...
  methods: {
    ...
    moveRight() {
      this.moveCounter >= this.reviews.length-1 ? this.moveCounter = 0 : this.moveCounter += 1

       for(let ref in this.$refs) {
         this.$refs[ref].style.transform = `translateX(-${272 * this.moveCounter}px)`
       }
    }, 
    moveLeft() {
    ...
    async popReview(reviewId){
      ...
      if(this.showReview){
        ...
        for(let ref in this.$refs) {
          this.$refs[ref].style.transform = `translateX(-${0 * this.moveCounter}px)`
        }
      } else {
